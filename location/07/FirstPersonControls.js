...
THREE.FirstPersonControls = function ( cam, domElement ) {
    ...
    this.removeControlCamera = false;
	...
	//	if ( this.moveBackward ) this.cam.translateZ( actualMoveSpeed );
	   ...
	   // if ( this.moveUp ) this.cam.translateY( actualMoveSpeed );
		...
		if (this.removeControlCamera == false)
		{	
		    var targetPosition = this.target,
		     	position = this.cam.position;
			
	     	targetPosition.x = position.x + 2 * Math.sin( this.phi ) * Math.cos( this.theta );
		    targetPosition.y = position.y + 2 * Math.cos( this.phi );	
		    targetPosition.z = position.z + 2 * Math.sin( this.phi ) * Math.sin( this.theta );

		     this.cam.lookAt( targetPosition );
		}

 		if (this.removeControlCamera == true)
		{       		
